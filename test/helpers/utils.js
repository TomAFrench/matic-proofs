...
export const encodeStateSyncerData = (user, rootToken, amount) =>
  `0x${user.slice(2).padStart(64, "0")}${rootToken.slice(2).padStart(64, "0")}${amount.toString(16).padStart(64, "0")}`;
...
export const constructERC1155DepositData = (ids, amounts) =>
  abi.encode(
    ...
    [ids.map(i => i.toString()), amounts.map(a => a.toString()), ["0x0"]],
  );
...
export function assertBigNumberEquality(num1, num2) {
  ...
  assert(num1.eq(num2), `expected ${num1.toString(10)} and ${num2.toString(10)} to be equal`);
}

export function generateFirstWallets({ mnemonics = packageJSON.config.mnemonics, n = 10, hdPathIndex = 0 }) {
